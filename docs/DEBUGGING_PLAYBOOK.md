...
| `service`   | string | `web` \| `control-plane` \| `sandbox-runtime` \| `slack-bot` \| `github-bot` \| `linear-bot` |
...
| `sandbox_id`          | Sandbox lifetime  | Control-plane on create/restore |
...
A prompt run flows through the client, control plane, and sandbox runtime (the Cloudflare
Container). For Slack, the path is: **slack-bot -> control-plane -> sandbox (sandbox-runtime)**.

1. Slack-bot generates `trace_id` and sends it as `x-trace-id` header.
2. Control-plane router propagates `trace_id` into Durable Object and sandbox provider calls.
3. The sandbox-runtime bridge binds the same trace to sandbox startup logs where supported.
...
#### Cloudflare Sandbox Provider (`component: "cloudflare-provider"`)

The `@cloudflare/sandbox` stub is a synchronous Durable-Object call, not a REST client, so there is
no generic per-call `*.request` event. Notable events:

| Event                                          | Level | Key Fields               | Description                                         |
| ---------------------------------------------- | ----- | ------------------------ | --------------------------------------------------- |
| `cloudflare.environment_image_build_triggered` | info  | `sandbox_id`, `image_id` | Environment image build kicked off                  |
| `cloudflare.delete_provider_image_unsupported` | warn  | `sandbox_id`             | Image deletion requested but unsupported by the SDK |
| `cloudflare.preview_domain_not_configured`     | warn  | `sandbox_id`             | `SANDBOX_PREVIEW_DOMAIN` unset, preview URL skipped |

---

### Sandbox Runtime (`service: "sandbox-runtime"`)

Logs emitted from inside the sandbox container (code bundled from `packages/sandbox-runtime/` and
installed via `packages/control-plane/Dockerfile`).
...
service="sandbox-runtime" msg="prompt.run" message_id="<MSG_ID>"
...
# 2. Did the provider create it?
service="control-plane" component="lifecycle-manager" msg="sandbox.spawn" sandbox_id="<SANDBOX_ID>"

# 3. Did startup succeed?
service="sandbox-runtime" msg="sandbox.startup" sandbox_id="<SANDBOX_ID>"

# 4. Did the bridge connect?
service="sandbox-runtime" msg="bridge.connect" sandbox_id="<SANDBOX_ID>"
...
Check the `error_type` and `error_message`. Then look at the lifecycle-manager spawn outcome:

```
service="control-plane" component="lifecycle-manager" msg="sandbox.spawn" outcome="error"
```
...
```
# Control-plane side
service="control-plane" msg="sandbox.restore" session_id="<SESSION_ID>" outcome="error"
```
...
Use the `trace_id` from step 1 to follow the request into the control-plane and sandbox-runtime
logs.
...
```
service="control-plane" msg="sandbox.spawn" | where duration_ms > 30000
```
...
```
service="sandbox-runtime" msg="opencode.crash"
...
- sig1 service signatures derived from the per-service `SERVICE_AUTH_SECRET`s
